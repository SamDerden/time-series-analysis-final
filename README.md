...
В ходе исследовательского анализа установлено, что временной ряд почасового энергопотребления PJME обладает ярко выраженной мультисезонной структурой:
...
Проверка стационарности с помощью теста Дики–Фуллера дала ADF Statistic = –18.83 и p‑value ≈ 2×10⁻³⁰, что значительно ниже 0.05. Это позволяет отвергнуть гипотезу о нестационарности и применять широкий класс моделей, включая те, которые требуют стационарности. Однако наличие сезонности требует её явного учёта при построении прогнозов, что подтверждается визуальным анализом декомпозиции ряда на тренд, сезонную и остаточную компоненты (периоды 24 и 168 часов). Таким образом, данные полностью подготовлены для дальнейшего моделирования, а выявленные закономерности формируют выбор моделей, способных обрабатывать сезонность и нелинейности.
---
...
Наилучший результат среди статистических методов показала модель Prophet, достигшая ошибки прогнозирования 4214.99 МВт. Однако её точность существенно уступает методам машинного обучения, что делает целесообразным их использование.
...
**Итог:** современные методы машинного обучения значительно превосходят классическую статистику на данном ряду, что подтверждает целесообразность их использования в энергетических задачах.
